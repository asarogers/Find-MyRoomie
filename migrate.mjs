...
async function uploadImage(url) {
  ...
  const uploadedAsset = await client.assets.upload('image', Buffer.from(arrayBuffer), {
    contentType,
  ...
  try {
    ...
    for (const post of blogPosts) {
      ...
      const sanityDoc = {
        ...
        tags: post.tags,
        mainImage: post.image ? await uploadImage(post.image) : undefined,
        sections: await Promise.all(post.sections.map(async (section) => ({
          _type: 'section',
          subtitle: section.subtitle,
          contentType: Array.isArray(section.content) ? 'list' : 'text',
          textContent: Array.isArray(section.content) ? undefined : section.content,
          listContent: Array.isArray(section.content) ? section.content : undefined,
          ...(section.image && {
            sectionImage: await uploadImage(section.image),
          }),
          ...(section.video && { sectionVideo: section.video }),
        }))),
      };
